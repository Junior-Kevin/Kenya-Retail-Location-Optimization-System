...
PURPOSE: Comprehensive Data Quality Assessment for Bronze Layer
DESCRIPTION:
    This script performs systematic data quality checks across all bronze layer
...
PRINT '===============================================================================';
PRINT 'DATA QUALITY ASSESSMENT: BRONZE LAYER';
...
WITH MarketAnalysis AS (
    ...
    LEFT JOIN bronze.competitor_stores_raw cs ON c.county = cs.county
...
WITH StoreValidation AS (
    SELECT 
        ...
        COUNT(*) as total_transactions
    FROM bronze.pos_raw p
    LEFT JOIN bronze.stores_raw s ON p.store_id = s.store_id
)
SELECT 
    CASE 
...
WITH QualityMetrics AS (
    ...
    SELECT 'CRM Duplicates' as category, 
           CASE WHEN (SELECT COUNT(*) - COUNT(DISTINCT customer_id) FROM bronze.crm_raw) > 0 
                THEN 'CRITICAL' ELSE 'LOW' END as severity, 
           1 as weight UNION ALL
    
    -- CRM Missing Emails
    SELECT 'CRM Missing Emails' as category, 
           CASE WHEN (SELECT COUNT(*) FROM bronze.crm_raw WHERE email IS NULL) > 
                     (SELECT COUNT(*) * 0.3 FROM bronze.crm_raw) 
                THEN 'HIGH' ELSE 'LOW' END, 
           2 UNION ALL
    
    -- Negative Transaction Amounts
    SELECT 'Negative Transaction Amounts' as category, 
           CASE WHEN EXISTS (SELECT 1 FROM bronze.pos_raw WHERE TRY_CAST(final_price_kes AS DECIMAL(18,2)) < 0) 
                THEN 'CRITICAL' ELSE 'LOW' END, 
           3 UNION ALL
    
    -- Invalid Transaction Dates
    SELECT 'Invalid Transaction Dates' as category, 
           CASE WHEN (SELECT COUNT(*) FROM bronze.pos_raw WHERE TRY_CONVERT(DATE, transaction_date) IS NULL) > 0 
                THEN 'HIGH' ELSE 'LOW' END, 
           2 UNION ALL
    
    -- Missing Geographic Coordinates
    SELECT 'Missing Geographic Coordinates' as category, 
           CASE WHEN (SELECT COUNT(*) FROM bronze.gis_locations_raw WHERE latitude IS NULL OR longitude IS NULL) > 0 
...
FROM (
    VALUES 
        (1, 'Fix negative transaction amounts in POS data', 'CRITICAL', 'UPDATE bronze.pos_raw SET final_price_kes = ABS(final_price_kes) WHERE final_price_kes < 0', '30 minutes'),
...
FROM (
    SELECT 'Data Completeness (CRM Email)' as metric_name,
           (SELECT 100.0 - ROUND(100.0 * SUM(CASE WHEN email IS NULL THEN 1 ELSE 0 END) / COUNT(*), 2) 
            FROM bronze.crm_raw) as current_value,
    ...
    SELECT 'Time Series Continuity',
           (SELECT ROUND(100.0 * COUNT(DISTINCT year_month) / 12.0, 2) 
            FROM bronze.economic_raw WHERE TRY_CAST(year AS INT) = 2023),
           100.0
) AS Metrics;
